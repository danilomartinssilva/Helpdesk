...
class SolicitacaoController extends AbstractActionController
{
            ...
            if (isset($request->filter)) {
                ...
                for ($i = 0; $i < count($filter); $i ++) {
                    ...
                    if ($filter[$i]['id'] == "buscaPorPeriodo") {
                        $where[] = " (viewsolicitacoes.{$filter[$i]['property']} between '{$filter[$i]['from']} 00:00:00' and '{$filter[$i]['to']} 23:59:00')  ";
                        /*print_r($where);*/
                    }
           ...
           $this->enviarEmailTecnico($insert, $cadsol->cd_cadser, $cadsol->cd_cadcli, $cadsol->data_cadsol);
            ...
            if ($insert > 0) {
                ...
                $gerarLog = $this->GerarLog()->log($this->getServiceLocator()
                		->get('Zend\Db\Adapter\Adapter'), 'admin', 'insert', 'solicitacao');
                /*
                 * Final Log
                */
    ...
    public function deleteAction ()
    {
        ...
        if ($auth->hasIdentity()) {
            ...
            if ($deleteSolicitacao) {
                 ...
                 */
                $gerarLog = $this->GerarLog()->log($this->getServiceLocator()
                		->get('Zend\Db\Adapter\Adapter'), 'admin', 'delete', 'solicitacao');
                /*
                 * Final Log
                */
            } else {
    ...
    {
        //Formata data e Hora//
        $dateHora = date_create($dateHora);
        $dateHora = date_format($dateHora, 'd-m-Y H:i:s');
